...
type client_op_spec =
  | Write of (string * string) (* key, value *)
  | Read of string (* key *)
  | SimulateTimeout
...
let mark_event_completed (engine : t) (id : event_id) : t =
  ...
  (* Update all children *)
  let children_ids =
    EventMap.find_opt id engine.reverse_dependencies |> Option.value ~default:[]
  in
  ...
  { engine with event_statuses = final_statuses; unmet_dependencies = new_deps }
